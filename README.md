...
| 🧠 知識校正層 | ✅ v3.0.24 (workspace) | AI 首輪分析前後同時使用知識增強：prompt 會注入 K1 facts / guidelines，並按 K1 public `v1.3.1` schema 以 `subject_head + panel_chair + all_roles` 組裝主任層事實；本地 `role_facts.json` 亦會按 topic 取角色事實注入 `【EDB學校管理知識中心角色事實】` 區塊；summary 現收斂為兩段式、較短、以通告主體為先，並避免不同通告被壓成同一模板或出現重複術語 |
